...
- Lucas-Kanade & Farneback Optical Flow Fusion Algorithms.
- ASC CDL Metadata Exchange Specifications.
"""
import os
...
# Initialize Shared System Library State
...
with st.sidebar:
    ...
    st.markdown("---")
    st.subheader("ASC CDL Parameters")
...
if uploaded_video:
    ...
    with c_left:
        ...
        if st.button("Step 1: Execute Unified Kinetic & Affective Analysis", type="primary"):
            ...
            # Spatial analysis sampling over 20-frame intervals
            for fno in range(0, total, 20):
                ...
                if prev_gray is not None:
                    # Calculate general optical flow tracking signals across the field
                    flow = cv2.calcOpticalFlowFarneback(prev_gray, gray, None, 0.5, 3, 15, 3, 5, 1.2, 0)
                    magnitude, _ = cv2.cartToPolar(flow[..., 0], flow[..., 1])
                    kinetic_energy = float(np.mean(magnitude))
                    
                    # --- UNIFIED LOOK SIGNALS MATRIX ---
                    # High motion signals map to active categories (Surprise/Excited).
                    # Micro signals drop back to low-arousal configurations (Calm/Neutral).
                    if kinetic_energy > 1.4:
                        assigned_recommendation = "surprise"
                        confidence_weight = min(kinetic_energy / 3.5, 1.0)
                    elif kinetic_energy > 0.4:
            ...
            # Process raw data samples into clean chronological timeline blocks
            f_segs = []
            if raw_timeline:
                ...
                for i in range(1, len(raw_timeline)):
                    if raw_timeline[i]['emo'] != curr['emo']:
                        ...
                        f_segs.append({
                            ...
                            "s_f": int(curr['s_f']), "e_f": int(end_f), "emotion": str(curr['emo']), "weight": float(curr['w_sum']/curr['cnt'])
                        })
                ...
                f_segs.append({
                    ...
                    "s_f": int(curr['s_f']), "e_f": int(total), "emotion": str(curr['emo']), "weight": float(curr['w_sum']/curr['cnt'])
                })
                
                st.session_state['unified_segments'] = f_segs
                st.session_state['unified_fps'] = fps

    # Render management UI panels if track elements exist
