    ...
    if uploaded_scene_video:
                ...
                if raw_scene_data:
                    ...
                    for i in range(1, len(raw_scene_data)):
                        if raw_scene_data[i]['emo'] != curr['emo']:
                            ...
                            curr = {"s_f": end_f, "emo": raw_scene_data[i]['f'], "w_sum": raw_scene_data[i]['w'], "cnt": 1}
                        else:
        ...
        if st.session_state['scene_segments']:
            with sc_right:
                ...
                st.write("Swap movement recommendations with your custom templates.")
                
                sc_transition_seconds = st.slider("🎬 Transition Smoothness (Seconds)", 0.0, 3.0, 1.0, 0.1, key="smoothness_slider_scenes")
                sc_transition_frames = int(sc_transition_seconds * st.session_state.get('scene_fps', 24.0))

                for idx, s in enumerate(st.session_state['scene_segments']):
                    with st.expander(f"Seg {idx}: {s['emotion'].upper()} (Motion: {round(s['weight']*100)}%)"):
                ...
                st.divider()
                if st.toggle("View Exportable CDL", key="toggle_xml_scenes"):
                    st.code(generate_cdl_xml(st.session_state['scene_segments']), language="xml")
                ...
                if st.button("Step 2: Render Master Preview", type="primary", key="btn_render_scenes"):
                    with st.spinner("Baking smoothed environmental grades into video..."):
                        ...
                        smoothed_parameters = []
                        for f_idx in range(total_frames):
                            start_window = max(0, f_idx - sc_transition_frames // 2)
                            end_window = min(total_frames - 1, f_idx + sc_transition_frames // 2)
